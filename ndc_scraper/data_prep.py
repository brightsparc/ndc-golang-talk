import re
import pandas as pd
from sklearn.model_selection import train_test_split

# Load the lines and set label and contents
df = pd.read_json('speakers.jl', lines=True)
df['talk_labels'] = df.talk.apply(lambda talk: ' '.join(["__label__%s" % re.sub('\W+', '', t).lower() for t in talk["tags"]])).values
df['talk_contents'] = df.talk.apply(lambda talk: ' '.join([talk["preamble"], talk["body"]])).values

# Train test split
train, test = train_test_split(df[['talk_labels', 'talk_contents']].dropna(), test_size = 0.2, random_state=42)

# Write back labels and conents
train.to_csv('train.txt', sep=' ', index=False)
test.to_csv('test.txt', sep=' ', index=False)
